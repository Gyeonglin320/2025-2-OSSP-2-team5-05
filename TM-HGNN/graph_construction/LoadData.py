import sys, os
sys.path.append(os.path.join(os.path.dirname(__file__), "prepare_notes"))
...
class LoadPaitentData():
    def __init__(self, name, type, data_type, tokenizer=''):
        ...
        self.dictionary = ["[PAD]", "[UNK]", "[CLS]", "[SEP]", "[MASK]"]
    ...
    def split_train_val_data(self, seed, ratio):
        ...
        cs = pd.Series([d.y_p.item() for d in self.train_set]).value_counts().to_dict()
    ...
    def get_train_test(self, batch_size, seed, ratio=0.8):
        ...
        self.test_set = PNGD(name=self.name, split='test', tokenizer=self.tokenizer, dictionary=self.dictionary, data_type=self.data_type)
        x = [data for data in self.test_set]
        print('load train data...')
        self.train_set = PNGD(name=self.name, split='train', tokenizer=self.tokenizer, dictionary=self.dictionary, data_type=self.data_type)
