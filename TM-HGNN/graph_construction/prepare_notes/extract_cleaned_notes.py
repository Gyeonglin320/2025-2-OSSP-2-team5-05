...
def clean_docs():
    ...
    for split in ['test', 'train']:
        ...
        patients = list(filter(lambda x: x.find("episode") != -1,
                               os.listdir(os.path.join(args.raw_path, args.task, partition))))
    ...
    for split in ['test', 'train']:
        ...
        for patient in tqdm(patients[:], desc=f'Iterating over patients in {args.task}_{partition}'):
            ...
            p_df.to_csv(os.path.join(args.raw_path, args.task, partition, patient),
                        sep=',', index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dimension', type=str, default='768',
                    help='input dimension (100 for word2vec, 768 for clinicalbert)')
    ...
    parser.add_argument('--split', type=str, default='train',
                    choices=['train','test'],
                    help='which split to clean (currently we still loop both)')
    parser.add_argument('--pre_path', type=str,
                        default='./data/DATA_PRE',
                        help='(not used in cleaning yet, but kept for consistency)')
